...
exprs [n] = [Val n]
exprs ns =
  [ e | (ls, rs) <- nesplit ns, l <- exprs ls, r <- exprs rs, e <- combine l r
  ]
...
results ns =
  [ res | (ls, rs) <- nesplit ns, lx <- results ls, ry <- results rs, res <- combine' lx ry
  ]
...
prop_lemma3 (xs, ys, zs) =
  ((xs, ys) `elem` nesplit zs)
    == (xs ++ ys == zs && ne (xs, ys))

prop_lemma4 :: ([Int], [Int], [Int]) -> Bool
prop_lemma4 (xs, ys, zs) =
  ((xs, ys) `elem` nesplit zs)
